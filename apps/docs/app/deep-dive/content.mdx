...
const editor = useEditor({
  extensions: [
    TwemojiExtension.configure({
      ...
      headerOptions: {
        randomButton: true,
        skinToneSelect: false,
      },
      customEmojiOptions: {
        maxSize: 5 * 1000 * 1000, // 5MB
      },
      navOptions: {
        minCellsToHideNav: 50,
      },
      gridOptions: {
        visibleRows: 6,
        cellSize: 38,
...
> ⚠️ You mus Invoke [`upload`](/deep-dive#upload)'s `handleSuccess` on a successful upload to run `onSuccess`
...
> ⚠️ You mus Invoke [`upload`](/deep-dive#upload)'s `handleError` on a successful upload to run `onError`
